\section{Pricing in Black-Scholes Model}
...
\]
\end{enumerate}
\subsection{Pricing Complex Options under Black Scholes Model: Treating the Underlying Asset as a Special Stock}
...
\Cref{subsect:bs-special-stock}: (European) power option and (geometric average
price) Asian option.
...
(matching with the expression in \labelcref{it:st-rn-expr}), with possibly
...
\begin{align*}
C_0^{\mathrm{geo-price}}&=e^{-rT}\expvq{(G_3-K)_{+}}\\
&=e^{-rT}\expvq{\qty(S_0\exp\qty[\qty(r-\delta^{*}-\frac{(\sigma^{*})^2}{2})(3)+\sigma^{*}\widetilde{W}_3]-K)_{+}} \\
&=\bs{S_0,\rc{\delta^*}}{K,r}{\rc{\sigma^*},T}.
\end{align*}

\item Some natural follow-up questions on \labelcref{it:bs-geo-avg-price} would
be the following.
\begin{enumerate}[label={Q\arabic*}]
\item \label{it:bs-geo-avg-strike-q} How about geometric average \emph{strike}
call option with payoff \((S_T-G_T)_{+}\) where
\(G_T=(\prod_{i=1}^{n}S_{ih})^{1/n}\) with \(T=nh\)? Can we use a similar
approach to price it?

\emph{Answer.} Although such call option can be priced, the approach used is
somewhat different. The approach in \labelcref{it:bs-geo-avg-price} does not
...
\Cref{subsect:exchange-opt}) for pricing it, so we defer the discussion on how
to price such call option to \labelcref{it:bs-geo-avg-strike}.
...
\faIcon{arrows-alt-h} ``exchanging'' the strike price (``pile of cash'')
\faIcon{money-bill-wave} for the asset
\item \emph{put:} selling an asset at a fixed strike price
\faIcon{arrows-alt-h} ``exchanging'' the asset for the strike price (``pile of
cash'') \faIcon{money-bill-wave}
\end{itemize}
...
another ``bundle'' (``bundle 1'') \faIcon{box-open} containing \(c_1\) units of
asset 1.
...
mentioned in \labelcref{it:bs-geo-avg-strike-q}, though the procedure would be
even more complicated than the one in \labelcref{it:bs-geo-avg-price}!
...
Like what we did in \labelcref{it:bs-geo-avg-price}, we are going to handle the
...
By \labelcref{it:bs-rn-pricing-fmla}, the geometric average strike call price
is then
\[
...
(stronger) assumption specified in \labelcref{it:blended-vol-fmla}, i.e.,
\(\{S_t^{(1)}\}\) and \(\{S_t^{(2)}\}\) are both geometric Brownian motions
individually with constant correlation coefficient \(\rho=\corr{\ln
S_t^{(1)}}{\ln S_t^{(2)}}\) always. Then by \labelcref{it:st-rn-expr} we can
write:
\[
...
\item By \labelcref{it:st-rn-expr}, we have
\[
S_3=S_0\exp\qty[\qty(r-\delta-\frac{\sigma^2}{2})(3)+\sigma\widetilde{W}_3].
\]
By \labelcref{it:bs-geo-avg-price}, we have
\[
...
To get \labelcref{eq:brown-cov-min}, we note that for any \(t=1,2\),
\begin{align*}
